...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
    ...
    if (!post) {
        return { title: 'Article non trouvé | Expert Pompe à Chaleur' };
    }

    return {
        title: post.seo_title || `${post.title} | Expert Pompe à Chaleur`,
        ...
        openGraph: {
            ...
            authors: [post.author_name || 'Expert Pompe à Chaleur'],
        },
    };
}
...
export default async function BlogPost({ params }: { params: Promise<{ slug: string }> }) {
    ...
    return (
        <main className="min-h-screen bg-white text-neutral-900 font-sans selection:bg-sky-100 selection:text-sky-900 pt-20">
             <script
            ...
            <nav className="container mx-auto px-4 py-4 border-b border-neutral-100">
                <ol className="flex items-center space-x-2 text-sm text-neutral-500">
                    <li><Link href="/" className="hover:text-sky-600 transition-colors">Accueil</Link></li>
            ...
            <header className="relative w-full h-[400px] bg-neutral-900 flex items-end">
                ...
                <div className="container mx-auto px-4 pb-12 relative z-10">
                   <div className="max-w-4xl">
                        {post.category && (
                            <span className="inline-block bg-sky-600 text-white text-xs font-bold px-3 py-1 rounded-full mb-4 uppercase tracking-wider">
                                {post.category.name}
                            </span>
                        )}
            ...
            <div className="container mx-auto px-4 py-12">
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
                    ...
                    <article className="lg:col-span-8">
                          {post.excerpt && (
                            <div className="bg-sky-50 border-l-4 border-sky-600 p-6 rounded-r-xl mb-10 text-lg font-medium text-sky-900">
                                {post.excerpt}
                            </div>
                        )}
                                ...
                                <div className="space-y-4">
                                     {post.faq.map((item, index) => (
                                        <details key={index} className="group bg-white rounded-xl border border-neutral-200 overflow-hidden">
                                            <summary className="flex items-center justify-between p-4 cursor-pointer font-semibold hover:text-sky-600">
                                                {item.question}
                    ...
                    <aside className="lg:col-span-4 space-y-8">
                        ...
                        <div className="bg-white border border-neutral-200 rounded-2xl p-6 shadow-sm">
                            ...
                            <div className="flex items-center gap-4">
                                <div className="w-12 h-12 rounded-full bg-sky-100 flex items-center justify-center text-sky-600 font-bold overflow-hidden relative">
                                    {(post.author_name || 'E')[0]}
                                      {/* Verified Badge */}
                                    <div className="absolute -bottom-1 -right-1 bg-white rounded-full p-0.5 shadow-sm">
                                        <div className="bg-sky-500 rounded-full w-3 h-3 border-2 border-white"></div>
                                    </div>
                                </div>
                                <div>
                                    <Link href={post.author_slug ? `/author/${post.author_slug}` : "#"} className="font-bold text-neutral-900 hover:text-amber-600 transition-colors">{post.author_name || 'Expert Pompe à Chaleur'}</Link>
                         ...
                         {/* CTA */}
                         <div className="bg-gradient-to-br from-sky-950 to-sky-900 text-white rounded-2xl p-8 text-center shadow-lg">
                            <h4 className="text-xl font-bold mb-2">Presupuesto Gratuito</h4>
                            <p className="text-sky-100 text-sm mb-6">Comparez les prix des installateurs RGE près de chez vous.</p>
                            <Link href="/devis" className="inline-block bg-white text-sky-900 px-6 py-3 rounded-full font-bold hover:bg-sky-50 transition-colors w-full">
                                Commencer
                            </Link>
                         </div>
                    </aside>
