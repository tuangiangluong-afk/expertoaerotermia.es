...
export function Footer({ config }: FooterProps) {
    ...
    const sitesByRegion = uniqueSites
        .filter(site => site.slug !== 'home' && site.slug !== 'expertpompeachaleur.com' && site.slug !== 'www.expertpompeachaleur.com')
        .reduce((acc, site) => {
    ...
    const getGlobalDiverseAnchor = (cityName: string, index: number) => {
        const variations = [
            `Aerotermia ${cityName}`,
            `Instalación Aerotermia ${cityName}`,
            `Calefacción RITE ${cityName}`,
            `Bomba de Calor ${cityName}`,
            `Presupuesto Aerotermia ${cityName}`,
        ...
        <footer className="bg-neutral-900 border-t border-white/10 py-12 text-neutral-400">
            <div className="container mx-auto px-4 text-center">
                ...
                <p className="max-w-2xl mx-auto text-sm mb-8">
                    {config.name} es el comparador líder para la instalación de sistemas de aerotermia y calefacción eficiente en España. Conectamos propietarios con instaladores homologados.</p>

                <div className="inline-flex items-center gap-2 bg-sky-900/30 border border-sky-800 px-4 py-2 rounded-full mb-8">
                    <span className="w-2 h-2 rounded-full bg-sky-500"></span>
                    <span className="text-sky-400 font-bold text-sm">Réseau d&apos;Installateurs Certifiés RGE QualiPAC</span>
                </div>
                    ...
                    <div className="grid md:grid-cols-4 gap-8 text-left max-w-7xl mx-auto">
                        ...
                        <div>
                            ...
                            <ul className="space-y-3 text-sm">
                                {config.slug === 'home' ? (
                                    <>
                                        <li><Link href="/ville/paris" className="text-neutral-400 hover:text-white transition flex items-center gap-2 group"><span className="w-1 h-1 rounded-full bg-neutral-600 group-hover:bg-sky-500 transition"></span>Île-de-France</Link></li>
                                        <li><Link href="/ville/lyon" className="text-neutral-400 hover:text-white transition flex items-center gap-2 group"><span className="w-1 h-1 rounded-full bg-neutral-600 group-hover:bg-sky-500 transition"></span>Auvergne-Rhône-Alpes</Link></li>
                                        <li><Link href="/ville/marseille" className="text-neutral-400 hover:text-white transition flex items-center gap-2 group"><span className="w-1 h-1 rounded-full bg-neutral-600 group-hover:bg-sky-500 transition"></span>Provence-Alpes-Côte d&apos;Azur</Link></li>
                                        <li><Link href="/ville/bordeaux" className="text-neutral-400 hover:text-white transition flex items-center gap-2 group"><span className="w-1 h-1 rounded-full bg-neutral-600 group-hover:bg-sky-500 transition"></span>Nouvelle-Aquitaine</Link></li>
                                        <li><Link href="/ville/toulouse" className="text-neutral-400 hover:text-white transition flex items-center gap-2 group"><span className="w-1 h-1 rounded-full bg-neutral-600 group-hover:bg-sky-500 transition"></span>Occitanie</Link></li>
                                        <li><Link href="/ville/nantes" className="text-neutral-400 hover:text-white transition flex items-center gap-2 group"><span className="w-1 h-1 rounded-full bg-neutral-600 group-hover:bg-sky-500 transition"></span>Pays de la Loire</Link></li>
                                    </>
                                ) : (
                                    <>
                        ...
                        <div>
                            ...
                            <ul className="space-y-3 text-sm">
                                <li>
                                    <Link href="/guides/subvenciones-aerotermia-2026" className="text-neutral-400 hover:text-white transition flex items-center gap-2 group">
                                        <span className={`w-1 h-1 rounded-full bg-neutral-600 group-hover:${theme.classes.bg} transition`}></span>
                                        Aides &amp; Subventions 2026
                                    </Link>
                                </li>
                                <li>
                                    <Link href="/guides/rentabilidad-aerotermia-calefaccion" className="text-neutral-400 hover:text-white transition flex items-center gap-2 group">
                                        <span className={`w-1 h-1 rounded-full bg-neutral-600 group-hover:${theme.classes.bg} transition`}></span>
                        ...
                        <div>
                            <h5 className="text-white font-bold mb-6 text-lg tracking-tight">Marcas Destacadas</h5>
                        ...
                        {Object.entries(sitesByRegion).map(([region, sites]) => (
                            <div key={region} className="space-y-4">
                                <h6 className="text-white/80 font-bold text-sm uppercase tracking-wider flex items-center gap-2">
                                    <span className="w-2 h-2 rounded-full bg-sky-500/50"></span>
                                    {region}
                                </h6>
                                <ul className="space-y-3 text-sm">
