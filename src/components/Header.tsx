...
export default function Header({
    ...
    const buttonColors = {
        ...
        rose: "bg-sky-600 hover:bg-sky-700 shadow-sky-500/20",
    };
                ...
                <div className="flex items-center gap-4">
                    ...
                    {isHub && (
                        <div className={`hidden md:flex items-center gap-6 text-sm font-medium ${variant === "light" ? "text-slate-300" : "text-slate-600"}`}>
                            <Link href="/guides" className="hover:text-sky-600 transition">Guías y Ayudas</Link>
                            <Link href="#simulateur" className="hover:text-sky-600 transition">Estudio Gratuito</Link>
                        </div>
                    )}

                    {/* RGE Badge (Desktop) */}
                    <div className="hidden lg:flex items-center gap-2 bg-sky-500/10 border border-sky-500/20 px-3 py-1.5 rounded-full">
                        <span className="w-2 h-2 rounded-full bg-sky-500 animate-pulse"></span>
                        <span className="text-xs font-bold text-sky-700">Instaladores Autorizados RITE</span>
                    </div>
