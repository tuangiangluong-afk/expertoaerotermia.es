        ...
        <div className="bg-slate-900 text-white p-6 rounded-2xl shadow-xl ring-1 ring-white/10 relative z-50">
            <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                    ...
                    <p className="text-sky-200 text-xs">Encuentre a su experto local</p>
                </div>
            </div>
            ...
            <form onSubmit={handleSearch} className="mb-4 relative">
                <div className="relative">
                    <input
                        ...
                        className="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-3 pl-10 text-sm focus:ring-2 focus:ring-sky-500 outline-none transition placeholder:text-slate-500 text-white"
                        autoComplete="off"
                    />
                    ...
                    <button
                        ...
                        className="absolute right-2 top-1/2 -translate-y-1/2 p-1.5 bg-sky-500 rounded-lg hover:bg-sky-400 disabled:opacity-50 transition text-slate-950"
                    >
                        {status === 'loading' ? (
                            ...
                            {filteredSuggestions.map((s) => (
                                <li key={s.slug}>
                                    <button
                                        ...
                                        className="w-full text-left px-4 py-3 text-sm text-slate-700 hover:bg-sky-50 hover:text-sky-700 transition flex items-center justify-between group"
                                    >
                                        <span className="font-medium text-slate-800">{s.name}</span>
                                        <ArrowRight size={14} className="opacity-0 group-hover:opacity-100 transition-opacity text-sky-500" />
                                    </button>
                                </li>
                            ))}
                        </ul>
                    ...
                    <div className="flex items-start gap-3">
                        ...
                        <div>
                            <p className="text-sm font-medium text-green-100 mb-2">
                                ¡Buenas noticias! Tenemos un instalador RGE asociado en <span className="text-white font-bold">{result.city}</span>.
                            </p>
                            <Link
                                href={result.url}
                ...
                <div className="bg-slate-800/50 border border-slate-700 rounded-xl p-4 animate-in fade-in slide-in-from-top-2">
                    <div className="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <Link
                                href="/#simulateur"
                                className="text-xs font-bold text-sky-400 hover:text-sky-300 underline decoration-sky-400/30"
                            >
